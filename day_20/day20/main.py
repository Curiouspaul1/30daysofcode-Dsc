...
from day20 import bcrypt,mongo
from day20.extensions import emailcheck
...
class User(mongo.Document):
    ...
    names = mongo.AnythingField()
    occupation = mongo.StringField()
    LastLogin = mongo.DateTimeField()
...
def signup():
    ...
    if emailcheck(user_data["email"]):
        ...
        new_user = User(username=user_data['username'],password = password_hash,email = user_data['email'],signup_date=d.datetime.utcnow(),occupation=user_data['occupation'],names=user_data['names'])
        new_user.save()
    else:
...
@main.route("/getuser/<id>")
@token_required
def getuser(active_user,id):
    ...
    time = date_.split(' ')
    if result:
        return jsonify({"_id":str(result.id),"username":result.username,"email":result.email,"date":str(date).split(' ')[0],"time":f"{time[1]} ".split('.')[0] + f" {date.strftime('%p')}","occupation":result.occupation,"names":result.names,"lastLogin":result.LastLogin})
